...
// PNGLoader 是 SourceEmbed 类型 spec 的 PNG 字节加载函数。
// 由消费者（yhbox 根包）注入，避免 vision 包反向依赖 yhbox。
var PNGLoader func(name string) ([]byte, error)

// LoadPNGForSpec 按 spec.Source 决定从哪里读 PNG 字节流：
//   - SourceEmbed: 调用 PNGLoader（消费者注入，通常是 yhbox.AssetBytes）
//   - SourceUser:  os.ReadFile("templates/" + spec.File)，路径相对 exe 工作目录
func LoadPNGForSpec(spec TemplateSpec) ([]byte, error) {
	switch spec.Source {
	case SourceEmbed:
		if PNGLoader == nil {
			return nil, fmt.Errorf("vision.PNGLoader 未注入；调用方应在 init 里设置")
		}
		return PNGLoader(spec.File)
	case SourceUser:
		return os.ReadFile(filepath.Join("templates", spec.File))
	default:
