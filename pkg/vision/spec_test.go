...
func TestLoadPNGForSpec_UserSourceReadsFromTemplatesDir(t *testing.T) {
	// 创建临时 templates/ 目录模拟 exe 同目录
	dir := t.TempDir()
	// 切到该目录，让 LoadPNGForSpec 的相对路径生效
	wd, _ := os.Getwd()
	defer os.Chdir(wd)
	os.Chdir(dir)

	tplDir := filepath.Join("templates", "fish")
	if err := os.MkdirAll(tplDir, 0755); err != nil {
		t.Fatal(err)
	}
	pngBytes := []byte{0x89, 'P', 'N', 'G'}
	if err := os.WriteFile(filepath.Join(tplDir, "x.png"), pngBytes, 0644); err != nil {
		t.Fatal(err)
	}

	spec := TemplateSpec{File: "fish/x.png", Source: SourceUser}
	got, err := LoadPNGForSpec(spec)
	if err != nil {
		t.Fatalf("err=%v", err)
	}
