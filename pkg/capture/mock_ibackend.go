...
func (b *mockBackend) Frame(hwnd Handle) (img *image.RGBA, err error) {
	defer func() {
		if r := recover(); r != nil {
...
func (b *mockBackend) FrameROI(hwnd Handle, x, y, w, h int) (img *image.RGBA, err error) {
	defer func() {
		if r := recover(); r != nil {
...
func (b *mockBackend) ClientSize(hwnd Handle) (int, int, error) {
	if hwnd == 0 {
