...
//
// logger 可为 Nop logger（独立调用场景）。返回 error 时调用方记日志即可。
func SwitchTeam(ctx context.Context, hwnd win.HWND, det *Detector, logger zerolog.Logger, target int) error {
	...
	logf := func(format string, args ...any) {
		logger.Info().Str("tag", "SYSTEM").Msgf(format, args...)
	}
