...
func NewPlayer(hwnd win.HWND, cfg *Config, logger *log.Logger, ctrl runctl.Control) *Player {
	return &Player{hwnd: hwnd, cfg: cfg, log: logger, ctrl: ctrl}
}

// Seek 请求跳转到 targetMs。Play 主循环在每次迭代顶端消费请求。
...
func (p *Player) Play(ctx context.Context, events []NoteEvent, progress func(curMs, totalMs int64)) {
	...
	p.log.Log(log.SYSTEM, "开始演奏 (mode=%s, 自动偏移=%+d, 手动偏移=%+d, %d 音符, 总时长 %.1fs)",
	...
	for i := 0; i < len(events); {
		if p.ctrl != nil {
			if p.ctrl.IsBack() {
				p.log.Log(log.SYSTEM, "停止演奏")
				return
			}
			if p.ctrl.IsPaused() {
				p.log.Log(log.SYSTEM, "已暂停")
				pausedAt := time.Now()
				if !p.ctrl.WaitUnpause() {
					return
				}
				start = start.Add(time.Since(pausedAt))
				p.log.Log(log.SYSTEM, "已恢复")
			}
		}
		...
		if p.seekPending.CompareAndSwap(true, false) {
			...
			p.log.Log(log.SYSTEM, "跳转到 %d:%02d", ms/60000, (ms/1000)%60)
			if i >= len(events) {
				break
			}
			continue
