		...
		if frame2, err := capture.Frame(m.hwnd); err == nil {
			...
			if hit, ok := m.det.WarehouseFull(frame2); ok {
				...
				if m.cfg.AutoSell.Load() {
					...
					if m.ctrl != nil {
						m.ctrl.Pause()
						m.log.Warn().Str("tag", "SYSTEM").Msg("鱼仓已满，自动出售已禁用，已自动暂停。手动卖完后在 GUI 点[继续]，或点[停止]退出。")
						if !m.ctrl.WaitUnpause() {
							return
						}
						m.log.Info().Str("tag", "SYSTEM").Msg("已恢复（state 留在 IDLE 继续扫描）")
					}
		...
		if m.routePhase(ctx, phase) {
			...
			if phase == PhaseReady {
				...
				if m.emptyCastStreak >= maxEmptyCastStreak {
					...
					if m.ctrl != nil {
						m.ctrl.Pause()
						m.log.Warn().Str("tag", "SYSTEM").Msg("连续空抛超限，已自动暂停。手动处理后在 GUI 点[继续]，或点[停止]退出。")
						if !m.ctrl.WaitUnpause() {
							return
						}
