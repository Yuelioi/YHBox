...
func newMachine(hwnd win.HWND, cfg *Config, det *Detector, logger *log.Logger, ctrl runctl.Control, statsHook StatsHook) *machine {
...
func (m *machine) run(ctx context.Context) Stats {
	...
	m.publishStats()
	m.log.Log(log.SYSTEM, "状态机启动 hwnd=0x%X", uint32(m.hwnd))
	...
	for {
		if ctx.Err() != nil {
			m.log.Log(log.SYSTEM, "收到取消信号，退出")
			return m.stats
		}

		if m.ctrl != nil {
			if m.ctrl.IsBack() {
				m.log.Log(log.SYSTEM, "已停止")
				return m.stats
			}
			if m.ctrl.IsPaused() {
				input.ReleaseAll(m.hwnd)
				m.log.Log(log.SYSTEM, "已暂停（GUI 点[继续]恢复 / [停止]退出）")
		...
		if err != nil {
			m.log.Log(log.SYSTEM, "抓帧失败 (%v)，2s 后重试", err)
...
func (m *machine) logState(format string, args ...any) {
	m.log.Printf("[%s] "+format, append([]any{m.state.String()}, args...)...)
}
