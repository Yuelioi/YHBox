...
// 只测试 runFlow 引擎和 waitDur 这种纯逻辑原语。log 为 nil，runFlow 内部 logState 调用要容忍 nil。
...
// hwnd=0 的测试 machine 抓帧直接失败，没法走到 detect/click 路径。这两个函数靠
// 真游戏窗口的集成验证（见 watchdog plan Task 10 手测）。
