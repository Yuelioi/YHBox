...
2. **window-control（并行新线）**：design + plan 就绪（[design.md](work/window-control/design.md) / [plan.md](work/window-control/plan.md)）→ 按 plan 执行序 A1→B1→B2→A3→A2→B3→B4→B5→C1→C2→C3→C4→C5→D1→D2→D3 实现 → 全绿 + 真机 smoke → 移 cold store。
