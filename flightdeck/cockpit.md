...
- [work/fishing-v2-rebuild/](work/fishing-v2-rebuild/) — **设计已写，待用户审阅后实施**。按当前节点系统和 package 四件套重建 `bin/data/containers/fishing-v2/` 自动钓鱼容器；旧 `container.json` 只作业务流程参考；默认没钱买鱼饵自动卖鱼，用户可用 `autoSellWhenNoCurrency=false` 改成直接结束。
...
3. **fishing-v2 重建** —— 用户审阅 `work/fishing-v2-rebuild/design.md` 后，写实施计划并执行容器数据重建。
4. **容器 package schema 收尾** —— 按阶段 7 做人工检查：新建容器四件套、列表字段/筛选/持久化、导出包不泄露本机绑定、lock hash 随内容变化。
5. **发布/推送决策** —— 若剩余 smoke 无问题，再 `git push origin main`。
