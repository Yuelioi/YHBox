...
- [work/point-px-unit/](work/point-px-unit/) — **设计定稿、spec 已 commit，待用户 review spec → writing-plans**(brainstorming HARD-GATE 内，未动代码）。Point 坐标加 Unit(%/px）+ 截图取点搬进 PointWidget；4 个坐标节点(ClickAt/Scroll/MouseMoveTo/Swipe）统一到单个 Point pin；px→ratio 只在新增 `InputService.ResolvePoint`(inputAdapter，有 hwnd）一处换算，backend/Drag/检测节点不动。用户已拍 MouseMoveTo 一起纳入。设计 [work/point-px-unit/design.md](work/point-px-unit/design.md)。
...
0. **point-px-unit:用户 review spec**([work/point-px-unit/design.md](work/point-px-unit/design.md))→ 批准后 invoke `superpowers:writing-plans` 出实现计划 → 逐 task TDD。当前活跃线。
