...
**Last updated**: 2026-06-15 by 月离 (本会话: Spec C Part 2 前端 done(待 smoke) + chrome 重构 — 侧栏删→标题栏全局导航 · 计划/关于页统一设置卡片 · 中文字体微软雅黑+pin 12px · 画布输出标签翻译 · 悬浮窗积木式(LauncherBlock)重构。typecheck/344 测试/build 全绿。下一步: 真机 smoke)。
**Active focus**: **Spec C 输出自动捕获 — 待真机 smoke**([spec](specs/2026-06-15-output-auto-capture.md))。P1 后端 + P2 前端 done([P1](plans/2026-06-15-output-auto-capture-part1-backend.md) 816d4cb..89e16c1 · [P2](plans/2026-06-15-output-auto-capture-part2-frontend.md)), 验过即收官归档。**本会话另完成 chrome 重构**(标题栏全局导航 / 计划·关于卡片 / 中文字体 / 画布输出标签 / 悬浮窗积木式)— 一并待真机验。UI 升级 (Spec A+B) 已归档。**默认不 push**。
...
- ⚠ **本会话 chrome/UI 真机 smoke** — 跑 `task dev`: ① 标题栏全局导航(容器/计划/设置/关于 切换+高亮, 编辑器里点设置再点容器能跳回; 画布更宽); ② 计划编辑 + 关于页 = 设置卡片风格(卡片/图标标题/右对齐控件); ③ 节点中文清晰(微软雅黑 + pin 12px); ④ **悬浮窗积木式**: 设置→悬浮窗 用 容器/文字标题/水平·垂直分隔符 拼布局 → 悬浮窗实时出效果, 顶部有 🚀+「启动器」头(旧 launcherGroups 已弃, 启动器开局空, 需重建)。
...
- **App 壳重构 (本会话 2026-06-15, 已 commit 未真机验)**: 侧栏 `AppSidebar` + `useSidebarCollapsed` **已删** → 全局导航全进 `AppTitleBar`(左 品牌+容器/计划 · 右 launcher/设置/关于 + 窗控; 容器保留 jump-back-to-last-container)。chrome/设置/表单页统一 **bordered card 配方**(见 [ui.md](checklists/ui.md) §设置卡片配方; 别再用 Inspector 扁平 SectionHeader)。**悬浮窗启动器**模型换 `LauncherGroups`→`LauncherItems []LauncherBlock{type:container|label|hsep|vsep}`(积木式; `FloatingLauncherView` flex-wrap 渲染 + HudShell 图标标题; 旧分组数据弃, 需重建)。中文字体: 等宽栈加微软雅黑/PingFang(中文走黑体不回退宋体)。frameless 窗圆角在 Win10+wails3 做不了 → [incident](incidents/2026-06-15-frameless-window-rounded-corners-unsupported.md)。
