...
> ~~**明确不在范围**: 普通"进子图手动改一个节点 → Ctrl+Z" 仍**不可撤销**~~ → **已补 (2026-06-17, 用户验收后追加)**: `applyDraftMutation` 在子图里编辑时把活动子图也快照进历史 (复用本 spec 的 historyEngine sgState 机制), 手动改子图现也可撤销。不再需要"大重构"——撤销引擎建好后这只是顺手扩 applyDraftMutation。
