...
### Active · Done
- [2026-06-23-held-exec-outputs.md](2026-06-23-held-exec-outputs.md) — done — 框架数据流改进(UE 式 held output): exec 节点 fire 时自动把出口 Data 字段存进本次运行的输出缓存(nodeID.field→值), 下游数据线可从任意距离直连读取, 免 GetVar、免紧邻约束。pullDataPin 对 exec 出口字段改读缓存; 缓存在 applyCaptures 旁填; 收编单跳 exec-data + 移除 EXEC_DATA_NOT_ADJACENT 警告; capture+GetVar 保留(命名/跨域)。触发自 AI 结构化输出 red/white 多消费的繁琐。
