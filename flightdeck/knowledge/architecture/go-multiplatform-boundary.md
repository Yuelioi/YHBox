...
- `internal/services/container/runtime.RuntimeContext` 仍直接持有 legacy `pkg/input.Backend` 与 `pkg/capture.IBackend`，但窗口元数据已归属 `automation/target`，runtime core 不再直接 import `lxn/win` / `pkg/winutil`。
...
- 完整 Linux 非 main package 的项目内失败已收敛到 hotkey、calibration、recording、tools；runtime 仍同时理解 controller 与 legacy input/capture bootstrap，这条双路径尚未收敛。Wails alpha.91 的 GUI package 在 `CGO_ENABLED=0` 下本身也无法构建，宿主 GUI 与 backend portability 必须分开验收。
