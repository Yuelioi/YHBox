...
Production resolves the canonical data workspace as `<RootSet.Data>/workspace` before constructing providers. Development-era versioned siblings such as `workspace-3.1` are not inspected, renamed, merged, or deleted. Once a released layout needs upgrading, only the root-level migration engine may move it after preflight, backup, journal, and verification.
