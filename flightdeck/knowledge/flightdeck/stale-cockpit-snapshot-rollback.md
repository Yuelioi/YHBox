...
SUMMARY: 旧快照在 land commit 后倒灌盖回工作区 cockpit, preflight 只读文件不验 git 差点重跑已完成的活
READ WHEN: preflight 读完 cockpit 后发现内容跟 git log 最近 land commit 对不上时; 工作区 cockpit.md 有未提交改动且不是本会话写的时; 怀疑"下一步"指向的活其实早干完了时
