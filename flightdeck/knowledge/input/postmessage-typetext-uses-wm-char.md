...
READ WHEN: 改 InputText/TypeText / 排查「文本输入节点在后台窗口没生效但按键/点击生效」/ 给后端补文本输入 / review TypeText 走哪条路
RECHECK WHEN: TypeText 实现改动 / 默认后端从 postmessage 换走 / WM_CHAR 对某类窗口 (Chromium/Slate) 真机表现有新发现
...
## 根因
`InputText` → `ctx.Input().TypeText` → backend.TypeText。`PostMessageBackend.TypeText` **旧实现**直接 `return TypeText(hwnd, s)`, 而 pkg 级 `TypeText` (pkg/input/typetext.go) 走**全局 SendInput KEYEVENTF_UNICODE**, `func TypeText(_ win.HWND, ...)` 把 hwnd 直接丢弃 —— SendInput 注入到**真实持有键盘焦点的前台窗口**。
