...
但 `sendInputBackend.Drag` (pkg/input/sendinput.go) 直接调共享原语 `pkg/input.MouseDrag` (pkg/input/input.go), 而 MouseDrag 完全是 `FakeActivate + setCursorPos + postMessage(WM_MOUSEMOVE / WM_xBUTTONDOWN / WM_xBUTTONUP)` 实现。⇒ 选 sendinput 后端跑 Swipe, 拖拽实际走窗口消息, 这类游戏看不见; 无报错、单测也过 (拖拽测试全用 mock / PostMessage 路径)。
