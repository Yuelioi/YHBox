...
- **通道 B（worker 事件校验失败本地化）没真机端到端 smoke**: 要造得手改磁盘 `bin/data/containers/<id>/container.json` 删 WindowTarget 再走热键; 走事件通道 `d.Error` 是对象、不受 [[wails-dev-fetch-transport-flattens-error]] 影响, 有 Go 单测背书, 按需补.
