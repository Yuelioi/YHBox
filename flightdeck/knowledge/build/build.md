...
`pkg/platform`、`pkg/input`、`pkg/capture`、`internal/nodes/input`、`internal/nodes/io` 已进入 Linux/macOS 原生测试矩阵；input/capture 的非 Windows dependency graph 不得重新出现 `lxn/win`。
