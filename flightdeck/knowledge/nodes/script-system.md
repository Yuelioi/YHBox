...
- **TemplateGUID pin 人体工学** (2026-06-30): `Semantic:"TemplateGUID"` 或 `Widget.Kind:"template-picker"` 的 pin 自动使用全局模板资产列表补全 (`scriptTemplateItemsForPin`)；补全下拉显示模板 name/category/tags/GUID，实际插入 GUID。`Templates: ▮` 裸值位插 `["guid"]`，`Templates: [▮]` 插 `"guid"`，字符串内只补裸 GUID。放大脚本编辑器工具栏的「截图模板」复用 `openScreenPicker('template_save')`，保存成功后按当前光标上下文插入新 GUID。新增模板类节点只要 Spec 语义标对，不需要补编辑器特例。
- **exit 比较补全** (`scriptExitCompareContext` in `lib/editorSignature.ts`): 光标落在 `r.exit === ▮` / `r.exit !== ▮` 右侧时, 反查最近的 `const/let/var r = Kind(...)`, 再从 `Spec.outputs` 取该节点 exec 出口候选。标准出口生成 `Exit.*`; 非标准出口生成字符串字面量。新增普通节点只要 Spec 输出完整, 此提示自动跟随；新增全新标准出口名若想变成 `Exit.Xxx` 常量, 需要补常量映射。
