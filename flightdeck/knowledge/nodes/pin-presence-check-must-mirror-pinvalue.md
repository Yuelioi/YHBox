---
kind: trap
summary: "历史 3.0 PinValue 两级 fallback 陷阱；3.1 strict Source/config schema 不允许该兼容读取。"
activation: symptom
read_when: "仅在审查 3.0 Container PinValue 兼容行为或解释旧数据时"
---
# ⚠ pin 值的存在性判定必须镜像 PinValue 的两级回退

> 历史知识：3.1 不接受 literal/top-level 双读 fallback；Source binding、config schema 和 Compiler 必须对同一 canonical 形状 fail closed。
