...
READ WHEN: 写 / 改 zh.ts / en.ts 翻译 (尤其含 `{` `}` `|` `@` `$` / JSON literal / 管道符 / 邮箱样例); 派 subagent 批量写 i18n 前; 改 vite.config 的 unplugin-vue-i18n 配置; UI 显示 `{n}` 字面 placeholder 没替换 / 弹 SyntaxError 整个组件挂掉
RECHECK WHEN: 改 vite.config 的 unplugin-vue-i18n 配置 / 升级 vue-i18n / message-compiler 行为变化时
