...
<!-- AUTO:docs -->
- [ai-nodes.md](ai-nodes.md) — active — when_to_read: 改/排查 AI 节点（图里调 LLM）、结构化类型输出、vision 识图、Image 节点（Capture/SaveImage/LoadImage）、Provider 缓存前; 加 DynamicDataFields 类动态带类型输出机制时; 撞「AI 输出字段绑不上变量 / 结构化解析 Fail / 图像输入没喂进去 / 改连接后没生效」 — applies_to: [ai-node, llm, structured-output, DynamicDataFields, vision, node-image, Capture, SaveImage, LoadImage, AIProviderService, provider-cache, internal/nodes/ai, internal/nodes/image, internal/node/spec.go, internal/node/types.go, internal/services/llm, internal/services/container/configtypes.go]
...
- [held-exec-outputs.md](held-exec-outputs.md) — active — when_to_read: 设计/排查 exec 节点出口 Data 字段被下游数据线消费时（Fail.Code→Switch、AI 结构化输出 red/white 多消费、Capture.Image→AI vision）; 撞「数据线接了 exec 出口字段但下游取不到值」; 改 pullDataPin / routeResult / 数据线求值路径前 — applies_to: [held-output, exec-output-data-field, data-edge, pullDataPin, captureExecOutputs, execOutputs, node-data-flow, internal/services/container/runtime/data_pull.go, internal/services/container/runtime/dispatch_v5.go, internal/services/container/runtime/runner.go, internal/services/container/validate.go]
...
- [yt-scripting-console.md](yt-scripting-console.md) — active — when_to_read: 改 yt 脚本控制台 / 加或改 yt.* API / 写控制台批量改节点的脚本 / 碰"编辑器内批量改节点 + 撤销"机制 / 给控制台加补全前 — applies_to: [yt-console, scripting, bulk-edit, console, completion, undo, frontend/src/lib/ytConsole/executor.ts, frontend/src/lib/ytConsole/completions.ts, frontend/src/composables/containerEditor/useYtConsole.ts, frontend/src/composables/containerEditor/historyEngine.ts, frontend/src/components/containers/YtConsoleModal.vue]
<!-- /AUTO -->
