...
Current:
- 当前刀：完成 Phase61 全量验证并提交；下一刀直接推进 `NeedsTarget(kind=win32-window, capabilities=...)`，不做旧 `WindowTarget` alias。
