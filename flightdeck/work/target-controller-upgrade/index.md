...
Plan next slice: finish Android editor tooling UX and manual MuMu verification. Phase 73 routes picker screenshot capture through Android ADB, so point/rect/template/color picker modes can reuse the existing picker UI for Android-backed nodes. Android ADB discovery now tries common emulator ports when no online device is listed, Windows builds carry a bundled `platform-tools/adb.exe` fallback before PATH, Android normalized input corrects portrait `wm size` by current `SurfaceOrientation` for landscape MuMu, and workflows can switch MuMu app tabs by `AndroidStartApp` package launch instead of clicking emulator chrome. Remaining work is deciding whether `PixelAt` needs a new coordinate-bearing API instead of the current Win32 cursor semantics.
...
- Phase 73 代码/文档：调研 `blue_archive_auto_script` 模拟器/ADB 支持并记录 findings；新增完整编辑期 target resolver，`templateCaptureAdapter` 对 AndroidTarget 走 `AndroidADBController.Screenshot` 生成 picker 底图，Android picker 复用现有 `ScreenPickerView` 支持点/范围/模板/取色截图来源。Android discovery 在无在线设备时尝试常见模拟器 ADB 端口，并按 `SurfaceOrientation` 修正 MuMu `wm size=720x1280` 但实际横屏截图/输入为 `1280x720` 的坐标漂移。Windows 构建内置 adbutils 提供的 ADB 34.0.5 三件套并统一 `adbexec` 解析路径，优先环境变量/随程序 `platform-tools`，最后回退 PATH，且隐藏 ADB 子进程窗口避免 cmd 闪烁。新增 `AndroidStartApp` / `AndroidStopApp`，用 Android package 切换/关闭目标 app，MuMu 顶栏 tab 不作为产品 target。`PixelAt` 仍因 MouseHUD 缺坐标输入保留 Android unsupported 边界。
