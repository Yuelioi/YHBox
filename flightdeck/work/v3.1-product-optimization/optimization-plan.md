...
状态：已完成。阶段 A–E 均按阶段边界实施、批量验收并形成提交或本次最终提交。

## 1. 为什么单独开 Topic

现有 workflow-editor-capability-roadmap 的职责是“从旧系统迁移并补齐到 3.1”，里面已经混合了架构迁移、能力恢复、真机修复和大量历史结论。继续往里面追加产品体验优化会产生三个问题：

1. 已完成的迁移结论与新的真机否定结论互相覆盖。
2. “底层能力存在”容易继续被误写成“用户已经可用”。
3. Topic 过大后，恢复上下文、阶段验收和责任边界都不清楚。

因此新 Topic v3.1-product-optimization 只负责 3.1 的产品优化与可靠性收口。旧 Topic 保留为升级历史和架构迁移记录，不再承载新的大规模体验扩张。
