...
批次 A-H 的本地工程范围已完成。批次 D 的 platform seam 与原生 GUI compile gate 定义均已
落地；首次远端 runner 验证属于需要维护者权限的发布治理，不作为本 Finished Work 的开放项。
Go 版本已因 govulncheck 结果提升到 1.25.12；项目 LICENSE 仍为 source-available。
...
- [x] D — 平台依赖守卫与 adapter 收敛（本地实现与门禁定义完成；远端首跑转发布治理）
