...
- Script runtime 注入标准出口常量 `Exit.*`；转换器对标准节点出口生成 `Exit.Found` / `Exit.NotFound` / `Exit.Default` 等，Subgraph 用户出口名仍保留字符串。
- 全量扫描 `bin/data/subgraphs`: 20/20 可转，无剩余拒转项。
- goja JS 已支持循环、条件、异常和表达式；本轮 runtime 仅增加 `Exit.*` 语法糖，不迁移底层节点出口名。
...
- 2026-06-30 语法清理：fishing-v2 主脚本的标准节点出口比较已改为 `Exit.*`，旧子图时代的 `return { exit: "done" }` / `tryHookF().exit === "failed"` 已移除；`tryHookF()` 现在返回 boolean。
...
- `node ...` checked fishing-v2 script has no old `done/failed` return objects and no standard-exit string comparisons.
