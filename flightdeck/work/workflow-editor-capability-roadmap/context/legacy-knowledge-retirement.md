...
这些条目已在 summary/read_when 中标记为“历史 3.0”或“仅旧基线取证”，当前保留是为了 3.1 恢复期间查行为，不代表现行架构。R5 发布门禁通过后统一处理，阶段中不边做边删除证据。

退役动作只有两类：

- `delete-after-promotion`：先把仍有效的一般原则写进现行 contract/knowledge，再删除旧实现条目。
- `archive-or-delete`：如果仍被归档 Topic 引用则移动到 `flightdeck/archive/knowledge-3.0/` 并修正引用；没有依赖则删除。

不得把旧实现条目继续留在主动 `flightdeck/knowledge/` 中，也不得为了清理而破坏 archived recovery graph。
...
3. 有 archived dependency 的条目移动并修复引用；无依赖条目删除。
4. 运行 `flightdeck_check`，确保 recovery graph 完整。
5. 主动 Knowledge 搜索不再返回 Container、Expr、yt、旧 Pin DTO 或 virtual marker 作为现行规则。
6. 退役变更与 R5 发布文档在同一 Flightdeck checkpoint 中记录 exact knowledgeCount。
