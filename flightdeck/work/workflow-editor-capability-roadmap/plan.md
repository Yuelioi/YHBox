...
3.0 的必要产品能力在唯一 3.1 架构上恢复、重做或明确删除，并由 capability ledger、golden
journeys、完整门禁和真实 Windows 宿主证据闭环。

## Current stage

Complete. R0–R5 与历史交付 1–44 均已完成；新的体验扩张转入独立 Work。
...
历史任务的 completion、blocked-by、verification 和 out-of-scope 保留在 `references/`；[任务 27](references/27-architecture-recovery.md) 是恢复设计总览。当前完成状态以本计划与 [Work index](index.md) 为准。

## 历史实现证据

历史任务 1–26 只证明相应组件和批次曾实现，不能直接作为 3.1 发布证据。它们的可复用实现必须重新映射到 [capability ledger](references/capability-ledger.md)，并通过对应 [golden journeys](references/golden-journeys.md)。
