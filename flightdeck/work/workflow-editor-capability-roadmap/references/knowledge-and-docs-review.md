...
| `flightdeck/knowledge` 未标“历史 3.0”的条目 | 可复用实施约定和已验证 trap | 稳定知识；正文应说明适用范围与需要重新核验的条件 |
...
| Finished Work | 当时任务过程与证据 | 不自动升级为现行约定 |
...
15 条无依赖旧知识已删除；3 条 Finished Work 固定引用的历史证据保留原路径。它们不进入默认恢复，也不再把旧实现当现行规则。
