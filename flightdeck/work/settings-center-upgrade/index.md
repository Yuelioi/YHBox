...
悬浮启动器下一阶段按“固定收藏 Dock + 可搜索命令面板”的双层模型升级：
- 第一阶段先重做当前悬浮窗：默认使用可识别的紧凑命令行（图标、名称、运行状态、快捷键），保留紧凑网格作为可选视图；分组标题成为正式区段，不再依赖低层级分隔符堆布局。
- 抽取实际窗口与设置预览共用的 `LauncherSurface` / `LauncherItem`，让预览、密度、运行态和失效态保持同一实现。
- 补齐方向键、Enter、Esc、数字直达、输入即搜索，以及运行中/成功/失败反馈；条目较多时按搜索、最近使用、固定分组渐进展示。
- 设置页增加“配置健康”区：统计有效入口、过期入口、快捷键冲突；一键清理只删除引用已不存在容器的启动器入口，清理前列明范围，保存后提供撤销，不删除真实容器。
- 第二阶段再增加热键唤出的全容器命令面板与最近使用；悬浮窗保持为少量高频收藏，不把所有容器塞进常驻小窗。
