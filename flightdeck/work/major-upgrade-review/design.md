...
本次目标版本定义为 **Yotta 3.0**。它是一次格式、接口、装配和发布体系同时换代的 breaking release：
...
main.go
  └─ internal/appbootstrap          构造完整应用，返回可 Start/Close 的 Application
       ...
       │    ├─ schema               唯一持久化模型与 v3 JSON contract
       │    ├─ catalog              显式节点注册与生成 catalog
       │    ├─ compiler             validate + dependency closure + immutable Program
       │    └─ runtime              执行 Program；拥有内部 adapter 和 dispatch state
       ...
       └─ internal/security         MCP、脚本、文件/网络/进程能力策略
...
`compiler.Compile(Source) (Program, Diagnostics)` 是验证、节点解析、子图闭包、pin 类型和 capability assembly 的单一入口。`Program` 不可变，runtime 只执行 Program，不再运行时重复猜 schema 或修复输入。
...
- 不承诺第三方 Go plugin ABI。先把 in-tree 节点贡献路径做稳定、可生成、可测试。
