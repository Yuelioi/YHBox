...
when_to_read: 新增一个 wails 后端 service (前端要新 RPC) 前; 改服务注册 / bindings 生成链路时
applies_to: [service, wails, rpc, bindings, backend, go, main.go, backend-facade]
when_to_update: 服务注册方式 / bindings 生成命令 / backend.ts 门面约定变化时
---
...
3. **生成 bindings**: `go build ./...` 过编译后跑 `wails3 generate bindings -clean=true` (或下次 `task dev`/`task build` 自动)。产物在 `frontend/bindings/yotta/internal/services/<name>/` (gitignore, 不提交)。
