...
export default {
  ...
  node: {
    ...
    Scroll: {
      label: '鼠标滚轮',
      description: '在窗口里某个位置滚一下鼠标滚轮。位置用比例填（0 到 1，0.5、0.5 是正中间）。滚动量填几格，正数往前滚、负数往后滚。方向选"竖向"是上下翻页，选"横向"是左右平移。常用来翻列表、缩放、切武器。',
      example: '把背包列表往下翻：X、Y 对准列表区域，滚动量填 -3，方向选"竖向"，运行时就在那里往下滚三格。横向滚条：方向选"横向"，正数向右、负数向左。',
      input: {
        XRatio: { label: 'X 比例' },
        YRatio: { label: 'Y 比例' },
