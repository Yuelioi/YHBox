...
export default {
  ...
  workflow: {
    ...
    recording: {
      ...
      result_mode: '生成方式',
      mode_steps: '按键与点击节点',
      mode_trajectory: '完整轨迹 Clip',
      action_summary: '按键 {keys} · 点击 {clicks} · 移动 {moves} · 滚轮 {scrolls}',
      trajectory_hint:
        '检测到拖拽、鼠标转向、滚轮或无法无损展开的输入；将插入 Play Input Clip 节点，以保留完整时序与轨迹。',
  ...
  assets: {
    ...
    recording: {
      title: '输入录制',
      hint: '相对模式记录完整鼠标转向，绝对模式记录点击、移动、拖拽和按键。',
  ...
  settingsAutomation: {
    ...
    capture: {
      hint: '点击后会临时启用全局窗口捕获键。切换到目标窗口并按 F9（或快捷键中心配置的按键）；捕获完成、取消或超时后会立即释放，不会触发目标软件中的同名快捷键。',
      start: '捕获窗口',
