...
export default {
  ...
  node: {
    ...
    automation: {
      ...
      movePointer: { title: '移动指针', description: '把指针移动到配置目标内的指定坐标。' },
      scrollPointer: { title: '滚动指针', description: '在配置目标内执行滚动。' },
      dragPointer: {
        title: '拖拽指针',
        description: '在配置目标内执行按下、移动和释放。',
      },
      movePointerRelative: {
        title: '相对移动指针',
  ...
  macroEditor: {
    ...
    state_none: '无',
    empty: '宏中还没有动作',
    empty_hint: '添加按键、点击、滚轮或等待；高级事件可在原子视图中编辑。',
  ...
  settingsNetwork: {
    origins: {
      title: 'HTTP 目标',
      hint: '配置基础 URL、响应大小和超时；工作流通过槽位直接使用。',
      add: '添加 HTTP 目标',
      unnamed: '未命名 HTTP 目标',
      ...
      origin_label: '基础 URL',
      origin_hint: '支持 HTTP、HTTPS、路径、查询和用户信息，也可以连接本机或私网地址。',
      byte_limit_label: '最大响应字节数',
      byte_limit_hint: '响应超过此上限时会失败，不会进入工作流数据。',
      timeout_label: '超时（毫秒）',
      timeout_hint: '由此配置决定单次请求等待多久。',
      delete: '删除目标',
      empty: '尚未配置 HTTP 目标',
      empty_hint: '添加基础 URL 后，HTTP GET 节点即可通过槽位使用。',
      new_label: '新 HTTP 目标',
    },
    confirm: {
  ...
  settingsApplications: {
    profiles: {
      title: '应用配置',
      hint: '配置程序路径和启动参数；工作流通过槽位直接启动或终止。',
      add: '添加应用',
      name_label: '显示名称',
      slot_label: '配置槽位',
      slot_hint: '工作流持久引用该标识，保存后不可修改。',
      executable_label: '程序路径',
      executable_hint: '选择要运行的绝对路径，不做哈希或身份校验。',
      replace: '重新选择',
      arguments_label: '启动参数',
      arguments_hint: '每行一个参数，按当前配置直接传给程序。',
      arguments_placeholder: '--project\nD:\\Projects\\fixed.aep',
      delete: '删除应用',
      empty: '尚未配置应用',
      empty_hint: '选择程序路径后，工作流即可通过槽位启动或终止。',
      cancelled: '未选择桌面应用',
  ...
  settingsAutomation: {
    targets: {
      title: '自动化目标',
      hint: '配置 Windows 窗口、Android ADB 或 Browser CDP 目标；工作流通过槽位直接调用。',
      add: '添加窗口目标',
      ...
      backend_label: '输入后端',
      backend_hint: 'SendInput 会置前目标窗口；PostMessage 向匹配窗口的消息队列投递。',
      capture_backend_label: '截图后端',
      capture_backend_hint: '选择 GDI 或 Windows Graphics Capture。',
      ...
      window_title_match_hint: '标题会原样保存，包括首尾空格；动态标题可以使用正则。',
      window_title_match_exact: '精确匹配',
      window_title_match_regex: '正则匹配',
      window_selection_label: '多个窗口同时匹配时',
      window_selection_hint: '可以要求唯一匹配，或使用当前最上层的匹配窗口。',
    ...
    android: {
      ...
      discovery_hint: '通过配置的或随包附带的 ADB 发现设备，并把 serial 与包名写入目标配置。',
      refresh: '刷新设备',
      none_found: '没有发现 ADB 设备。请启动模拟器或连接设备，并允许 USB 调试授权。',
      device_label: 'ADB 设备',
      device_hint: '选择设备，或使用发现结果填入 serial。',
    ...
    browser: {
      ...
      discovery_hint: '填写任意 HTTP 或 HTTPS CDP endpoint，然后选择页面写入目标配置。',
      refresh: '发现页面',
      none_found: '没有发现页面。请确认浏览器调试接口已启动且 endpoint 可以访问。',
      endpoint_label: 'CDP endpoint',
      endpoint_hint: '支持本机、私网或远程 HTTP/HTTPS 地址。',
      page_label: '页面',
      page_hint: '选择页面 ID；运行时会从 endpoint 重新发现当前 WebSocket 地址。',
      url_label: '发现时 URL',
      state_label: '运行健康状态',
      not_checked: '尚未检查',
      check_health: '检查',
    ...
    capture: {
      hint: '点击后启用窗口捕获键；切换到目标窗口并按 F9（或快捷键中心配置的按键），结果会直接写入目标配置。',
      start: '捕获并启用窗口',
  ...
  settingsAI: {
    ...
    profiles: {
      ...
      provider_label: '接口协议',
      model_label: '精确模型名',
      model_hint: '按原样发送，不自动发现或替换别名。',
      endpoint_label: 'API 基础地址',
      endpoint_hint: '填写供应商 base URL；请求路径由接口协议决定。',
