...
const ARRAY_METHODS = ["filter", "forEach", "map", "find", "some", "every", "slice", "flatMap", "length"];
...
export function ytCompletionSource(ctx: CompletionContext): CompletionResult | null {
  ...
  if (kind) {
    ...
    return { from: w ? w.from : ctx.pos, options: words.map((label) => ({ label, type })), validFor: /^\w*$/ };
  }
  // 顶层显式触发 (Ctrl+Space) → 提示 yt。
  if (ctx.explicit) {
    const w = ctx.matchBefore(/[\w.]*$/);
    return { from: w ? w.from : ctx.pos, options: [{ label: "yt", type: "variable" }], validFor: /^[\w.]*$/ };
  }
  return null;
}
