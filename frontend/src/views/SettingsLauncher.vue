<template>
  <div class="p-4 sm:p-5 mx-auto w-full max-w-3xl space-y-4">
    <div>
    ...
    <div
      ...
      class="text-xs text-dimmed py-8 text-center border border-dashed border-default/60 rounded-lg"
    >
      还没有分组，点「新建分组」开始。
    </div>
    ...
    <div
      ...
      :key="g.id"
      class="rounded-lg border border-default/60 bg-elevated/20 p-3 space-y-2"
    >
      <div class="flex items-center gap-2">
        <UInput
          :model-value="g.name" size="sm" class="flex-1 min-w-0" placeholder="分组名（如 战斗）"
      ...
      <div v-if="g.items.length === 0" class="text-[11px] text-dimmed px-1 py-1">空分组 — 下面添加容器。</div>
      <VueDraggable v-else v-model="g.items" :animation="150" handle=".drag-h" class="space-y-1" @end="persist">
        <div
          v-for="it in g.items"
          :key="it.containerId"
          class="flex items-center gap-2 px-2 py-1.5 rounded-md border border-default/50 bg-default/40"
        >
