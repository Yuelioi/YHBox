      ...
      nodes: [
        {
          ...
          x: 0,
          y: 0,
          config: { literal: { VarName: 'x', Scope: 'auto' } },
        ...
        },
        {
          ...
          x: 0,
          y: 0,
          config: { literal: { VarName: 'x', Scope: 'local' } },
          createdAt: '2026-05-19T00:00:00Z',
        },
        {
          id: 'sv',
          ...
          x: 0,
          y: 0,
          config: { literal: { VarName: 'x', Scope: 'global' } },
          createdAt: '2026-05-19T00:00:00Z',
        },
      ],
      edges: [],
...
describe('useVarMutations', () => {
  ...
  it('renameVar: changes scope=auto/global/undefined nodes only, preserves scope=local', () => {
    ...
    const litOf = (id: string) =>
      (nodes.find((n) => n.id === id)!.config!.literal as Record<string, unknown>).VarName
  ...
  it('deleteVar(cascade=true): removes decl + non-local referencing nodes + their edges', () => {
    const draft = ref<Container>(makeDraft())
    draft.value.graph.edges = [{ from: 'gv1.value', to: 'sv.value' }]
    const m = useVarMutations(draft)
  ...
  it('VarLastChange: renameVar renames VarLastChange node config.literal.VarName', () => {
    ...
    const m = useVarMutations(draft)
    m.renameVar('hp', 'x')
    expect((draft.value.graph.nodes[0].config!.literal as Record<string, unknown>).VarName).toBe(
      'x',
    )
  })
        ...
        nodes: [
          {
            ...
            x: 0,
            y: 0,
            config: { literal: { VarName: 'hp', Scope: 'auto' } },
            createdAt: '2026-05-19T00:00:00Z',
          },
          {
            id: 'ct1',
