...
function mainDraft(): Container {
  return {
    ...
    graph: {
      id: 'g',
      schemaVersion: 1,
      nodes: [{ id: 'a', kind: 'Sleep', x: 0, y: 0, config: { literal: { JitterPct: 0 } } }],
      edges: [],
    },
    tags: [],
    createdAt: '',
...
function subA(): Subgraph {
  return {
    ...
    graph: {
      id: 'sgA',
      schemaVersion: 1,
      nodes: [{ id: 's1', kind: 'Sleep', x: 0, y: 0, config: { literal: {} } }],
      edges: [],
    },
    createdAt: '',
  } as unknown as Subgraph
}
const SPEC_PINS: Record<string, Record<string, PinType>> = {
  Sleep: { Duration: 'number', JitterPct: 'number' },
}
const DEFAULTS: Record<string, Record<string, unknown>> = {
  Sleep: { Duration: 1000, JitterPct: 0 },
}
...
describe('useYtConsole', () => {
  it('buildModel: 主图 + 子图节点都进, sgID/defaults/specPins 对', () => {
    ...
    expect(m.map((x) => [x.id, x.sgID])).toEqual([
      ['a', null],
      ['s1', 'sub-A'],
    ])
