...
describe('summarizeProblems', () => {
  ...
  it('error present -> fail', () => {
    expect(summarizeProblems([err('error'), err('warning')])).toEqual({ errorCount: 1, warnCount: 1, status: 'fail' })
  })
  it('only warnings -> warn', () => {
    expect(summarizeProblems([err('warning'), err('warning')])).toEqual({ errorCount: 0, warnCount: 2, status: 'warn' })
  })
})
