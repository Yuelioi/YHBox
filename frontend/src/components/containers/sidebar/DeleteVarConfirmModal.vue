...
<template>
  <BaseModal
    ...
    :title="t('var.delete.title', { name: varName })"
  ...
  >
    <div class="space-y-4">
      <p class="text-xs text-muted">
        {{ t('var.delete.refs_prefix') }} <strong class="text-warning">{{ refCount }}</strong>
        {{ t('var.delete.refs_suffix') }}
      </p>

      <div
        class="max-h-32 overflow-y-auto bg-elevated/40 rounded p-2 text-[10px] font-mono space-y-0.5"
      >
        <div v-for="id in refIDs.slice(0, 20)" :key="id" class="text-dimmed">{{ id }}</div>
        <div v-if="refIDs.length > 20" class="text-dimmed italic">
          {{ t('var.delete.more_count', { n: refIDs.length - 20 }) }}
        </div>
      </div>
    ...
    <template #footer>
      <div class="flex flex-col gap-2 w-full">
        <UButton color="primary" variant="soft" icon="i-tabler-shield" @click="onConfirm(false)">
          ...
          <template #trailing>
            <span class="text-[10px] text-dimmed ml-1">{{
              t('var.delete.mode_keep_desc_inline')
            }}</span>
          </template>
        </UButton>
        ...
        <UButton color="neutral" variant="ghost" @click="modelOpen = false">{{
          t('common.cancel')
        }}</UButton>
      </div>
    </template>
