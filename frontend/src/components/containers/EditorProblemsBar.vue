...
<template>
  <div class="shrink-0 border-t border-default bg-default">
    ...
    <div
      ...
      class="max-h-64 overflow-y-auto px-3 py-2 space-y-2"
    >
      <div
        ...
        :class="
          e.severity === 'error' ? 'border-error/40 bg-error/5' : 'border-warning/40 bg-warning/5'
        "
      >
    ...
    >
      ...
      <template v-else>
        <UBadge v-if="summary.errorCount > 0" size="xs" color="error" variant="soft">{{
          summary.errorCount
        }}</UBadge>
        <UBadge v-if="summary.warnCount > 0" size="xs" color="warning" variant="soft">{{
          summary.warnCount
        }}</UBadge>
