<template>
  ...
  <div v-else class="relative">
    ...
    <div
      v-if="open"
      class="absolute z-50 mt-1 w-full bg-default border border-default rounded shadow-lg max-h-60 overflow-y-auto"
    >
      <button
        v-for="v in filtered"
        :key="v.name"
      ...
      >
        <span>{{ v.name }}</span
        ><span class="text-[10px] text-dimmed font-mono">·{{ v.type }}</span>
      </button>
      <button
        v-if="showCreate"
        type="button"
    ...
    <button
      v-if="undeclared"
      type="button"
      class="text-[11px] text-warning underline mt-0.5"
      @click="onCreate"
    >
...
function pickExisting(v: { name: string; type: VarType }) {
  open.value = false
