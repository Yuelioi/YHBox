<template>
  <div class="shrink-0 h-11 px-3 border-b border-default flex items-center gap-2 bg-default/60">
    ...
    >
      ...
      <span>跑中</span>
      <span v-if="runningNodeKind" class="text-emerald-200/80">
        · {{ runningNodeLabel }}
      </span>
    </div>
    <UButton
      ...
      >停止</UButton
    >
    <!-- 游戏窗口/分辨率检测（编辑器内不用切回 dashboard 也能测分辨率） -->
    <GameStatusIndicator variant="inline" />
    ...
    <UButton
      ...
      :title="
        dirty ? '请先保存再试运行' : execStoreRunning ? '已有任务在跑，先停' : '入队运行一次'
      "
      @click="$emit('try-run')"
      >试运行</UButton
    >
    <UButton size="sm" color="primary" icon="i-tabler-check" :disabled="!dirty" @click="$emit('save')"
      >保存</UButton
    >
    <UButton
      size="xs"
