<!-- 共享代码编辑器主体 (语言无关) — 三处复用: EditorModal(Script/Expr 放大) + YtConsoleModal。
     调用方经 extensions() 注入语言 (高亮/补全/lint/hover, 通常 modal:true 档)。
     工具栏 (撤销重做 | 注释/查找 | 片段/扩展 | 整理缩进/折叠) + 编辑器 + 状态栏 + 右侧参考面板。
     v-model:modelValue 双向当前文本; @submit = Ctrl+Enter (宿主决定: EditorModal→确认 / 控制台→运行)。
     不含模态壳 / 确认·取消 / 运行·输出 —— 那些是宿主的事。 -->
<template>
  <div class="flex gap-3 min-h-0 h-full">
...
const props = defineProps<{
  modelValue: string
  /** 构建语言扩展 (高亮/补全/lint, modal 档); 不要带 onChange — 本组件统一发 update:modelValue。 */
  extensions: () => Extension[]
  reference?: RefItem[]
  /** 实时状态栏: 首条问题 (文案+位置, 点击跳转), null = 无问题。 */
  lintFirst?: (doc: string) => { message: string; from: number } | null
  /** 工具栏「注释」按钮 (语言有注释语法 — Script JS 开, Expr 关)。 */
  commentable?: boolean
  /** 工具栏「折叠」按钮组 (语言支持折叠 — Script 开, Expr 关)。 */
  foldable?: boolean
  /** 传了就有「片段」入口 (codeSnippets store 按语言取 + 新建/管理)。 */
...
const emit = defineEmits<{
  ...
  /** Ctrl+Enter — 宿主决定语义 (EditorModal→确认 / 控制台→运行)。 */
  submit: []
}>()

const host = ref<HTMLElement | null>(null)
const search = ref('')
const doc = ref(props.modelValue ?? '') // 当前文本镜像 (状态栏/统计用)
...
const statusError = computed<{ message: string; from: number } | null>(() =>
  props.lintFirst ? props.lintFirst(doc.value) : null,
)

const statsText = computed<string>(() => {
  const d = doc.value
  const lines = (d.match(/\n/g)?.length ?? 0) + 1
  return t('inspector.editor_status_stats', { lines, chars: d.length })
})

// 用户片段管理入口: 带非空选区 = 进新建表单预填; 无选区 = 进列表。
