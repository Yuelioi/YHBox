...
- **战斗 / 队伍切换（battle）**：全局热键一键切上阵队伍（修饰键 Ctrl / Ctrl+Shift / Ctrl+Alt / Shift+Alt / Ctrl+Shift+Alt 五选一 + 数字 1~6）；后台抓帧验证切换成功才退出编队 UI
...
- **自动钓鱼 / 店长（锤子）/ 弹琴**：在对应 tab 点 `开始` / `暂停` / `停止`
- **战斗**：启动全局热键后用 `Ctrl+Shift+1~6`（或自选的修饰键组合）切上阵队伍
- **设置**：日志显示开关 / 自动滚动 / 时间戳 / 状态标签 / 自动换行
- **帮助 / 关于**
...
6. 切队伍前需在游戏主界面、无弹窗占用 `L` 键（按 L 能正常打开编队 UI）
