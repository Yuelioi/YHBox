...
func (s *Server) runNode(ctx context.Context, kind string, params map[string]any, hwnd uintptr) (RunNodeResult, *node.Image) {
	...
	registry := node.SnapshotRegistry(s.deps.Registry)
	c, nodeID, err := buildMicroContainerWithRegistry(registry, kind, params)
	if err != nil {
	...
	if hasBlockingValidationErrorWithRegistry(registry, c) {
	...
	return runMicroContainerWithRegistry(ctx, registry, rt, c, nodeID)
}
...
func hasBlockingValidationError(c *container.Container) bool {
	return hasBlockingValidationErrorWithRegistry(node.DefaultRegistrySnapshot(), c)
}

func hasBlockingValidationErrorWithRegistry(registry node.RegistryReader, c *container.Container) bool {
	for _, e := range container.ValidateContainerWithRegistry(c, nil, registry) {
