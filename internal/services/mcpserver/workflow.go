...
func explainDiagnostic(request ExplainDiagnosticRequest) (ExplainDiagnosticResult, error) {
	...
	explanations := map[string]ExplainDiagnosticResult{
		...
		"EDGE_CHANNEL_MISMATCH":  {Explanation: "The edge channel does not match the source output and target input channels.", Repairs: []string{"Connect data to data, exec to exec, or error to error."}},
