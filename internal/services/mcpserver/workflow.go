...
func explainDiagnostic(request ExplainDiagnosticRequest) (ExplainDiagnosticResult, error) {
	...
	explanations := map[string]ExplainDiagnosticResult{
		...
		"INVALID_INSTRUCTION_PLACEMENT": {Explanation: "A host-lowered node instruction is placed in a graph where its lifecycle semantics are invalid.", Repairs: []string{"Keep run-root nodes in the main entry graph; use graph entries for reusable subgraphs."}},
