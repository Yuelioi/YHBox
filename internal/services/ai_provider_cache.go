...
func NewAIProviderCache(get SettingsGetter) *AIProviderCache {
	return &AIProviderCache{get: get, cache: map[string]aiCacheEntry{}}
}
...
func (c *AIProviderCache) Provider(connectionID string) (llm.Provider, error) {
	...
	p, err := llm.New(llm.ConnectionConfig{Protocol: conn.Protocol, BaseURL: conn.BaseURL, APIKey: conn.APIKey})
	if err != nil {
		return nil, err
	}
...
func connFingerprint(c *AIConnection) string {
	h := sha256.Sum256([]byte(c.Protocol + "\x00" + c.BaseURL + "\x00" + c.APIKey))
