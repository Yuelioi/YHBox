...
func TestConnectionListsModels(t *testing.T) {
	...
	defer srv.Close()
	res := NewAIService().TestConnection(TestConnReq{Connection: AIConnection{Protocol: "openai", BaseURL: srv.URL, APIKey: "k"}})
	if !res.Ok {
...
func TestConnectionFallbackChat(t *testing.T) {
	...
	res := NewAIService().TestConnection(TestConnReq{
		Connection: AIConnection{Protocol: "openai", BaseURL: srv.URL, APIKey: "k"},
		TestModel:  "m",
	})
	if !res.Ok {
