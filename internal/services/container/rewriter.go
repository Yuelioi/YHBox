...
// GraphRewriter 统一 graph 改写抽象（v2 spec §2.4，GPT 第一轮）。
//
// 调用点（v2 spec）：
//   - Copy-on-use 库 → 容器：subgraph clone 时所有内部节点 id 重发；template key 冲突时 rename + patch；
//     SubgraphOutputDecl ID 也要重发 → SubgraphOutput 节点 config.declID 必须同步 patch (Gemini review B-3)
...
func (r *GraphRewriter) Apply(g *Graph) {
	...
	for i := range g.Nodes {
		...
		// 3. SubgraphOutput 节点 config.declID 改写（B-3 修复）
