...
func (r *ContainerRunner) execROIColorScan(ctx context.Context, n *container.GraphNode, tok ExecToken) ([]ExecToken, error) {
	...
	// v4: all numeric thresholds via data-in pin.
	minCluster := int(r.pullNumber(n, "minClusterPx", 2))
	maxCluster := int(r.pullNumber(n, "maxClusterPx", 0))
	if maxCluster <= 0 {
		if axis == "x" {
			maxCluster = roi.W / 3
		} else {
	...
	minClusterCount := int(r.pullNumber(n, "minClusterCount", 1))
