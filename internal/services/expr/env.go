...
// 不暴露 $vars / $sys / $params 命名空间. 用作 Expr.Eval(InputEnv(inputsMap)).
//
// 设计意图: 让 Expr 节点表达式跟 v3 expr 隔离 — Expr 输入必须显式声明在 inputs[],
// 想引用变量/sys/入参 → 拖 GetVar/GetSys/GetParam 节点把值送进 Expr.
// 这样表达式不再隐式依赖 runtime 上下文, 可视化+确定性 (spec §5).
type InputEnv map[string]Value

func (e InputEnv) Get(name string) (Value, error) {
	// $-prefixed paths are NOT supported in v4 Expr node context — return nil (treated as null).
