...
func TestStateReadIsBoundFromProgramStateAndJournaledAsAnEffect(t *testing.T) {
	...
	definition, _ := builtins.Definition(nodes31.StateReadNodeID)
	...
	source := []byte(fmt.Sprintf(`{
		...
		"revision":0,"entryGraph":"main","graphs":[{"id":"main","kind":"main","nodes":[{
			"id":"read","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":0,"y":0},
	...
	for _, fact := range journal.Current().Journal() {
		if fact.Kind == run31.JournalAdapterAction && fact.EffectID == nodes31.StateReadEffectID && fact.ActionOutcome == run31.ActionSucceeded {
			found = true
		}
	}
	if !found {
