...
func TestApplicationLifecycleUsesInstalledTargetAndRedactsJournal(t *testing.T) {
	...
	capabilityDefinition, ok := builtins.Catalog.LookupCapability(nodes31.ApplicationLifecycleCapabilityID)
	if !ok {
...
func applicationSource(t *testing.T, builtins nodes31.Builtins, slot string) []byte {
	t.Helper()
	started, _ := builtins.Definition(nodes31.RunStartedNodeID)
	launch, _ := builtins.Definition(nodes31.LaunchApplicationNodeID)
	terminate, _ := builtins.Definition(nodes31.TerminateApplicationNodeID)
	return []byte(fmt.Sprintf(`{
		...
		"graphs":[{"id":"main","kind":"main","nodes":[
			...
			{"id":"launch","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},"config":{"slot":%q},"bindings":{}},
			{"id":"stop","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":2,"y":0},"config":{"slot":%q},"bindings":{}}
		],"edges":[
