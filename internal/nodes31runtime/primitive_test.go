...
func TestPrimitiveInlineAdaptersCompileAndExecuteNominalValues(t *testing.T) {
	...
	add, _ := builtins.Definition(nodes31.AddNodeID)
	greater, _ := builtins.Definition(nodes31.GreaterThanNodeID)
	source := []byte(fmt.Sprintf(`{
		"format":"yotta.workflow","version":"3.1","workflow":{"id":"wf-primitives","name":"Primitives"},
		"revision":0,"entryGraph":"main","graphs":[{"id":"main","kind":"main","nodes":[
			{"id":"add","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":0,"y":0},"config":{},
			 "bindings":{"a":{"kind":"value","value":2},"b":{"kind":"value","value":3}}},
			{"id":"greater","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},"config":{},
...
func TestPrimitiveUnrepresentableResultPersistsDeclaredTerminalFailureWithoutInventedPort(t *testing.T) {
	...
	source := []byte(fmt.Sprintf(`{
		"format":"yotta.workflow","version":"3.1","workflow":{"id":"wf-overflow","name":"Overflow"},
		"revision":0,"entryGraph":"main","graphs":[{"id":"main","kind":"main","nodes":[{
			"id":"add","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":0,"y":0},"config":{},
	...
	if runErr == nil || journal.Current().Status() != run31.StatusFailed || !failed || failure.Code != "math.result_not_representable" {
...
func compilePrimitiveProgram(t *testing.T, builtins nodes31.Builtins, source []byte) compiler.ProgramSnapshot {
	t.Helper()
