...
func TestPlayInputClipReadsNominalBlobAndUsesExclusivePlaybackSession(t *testing.T) {
	...
	playbackCapability, ok := builtins.Catalog.LookupCapability(nodes31.AutomationPlaybackCapabilityID)
	if !ok {
	...
	for _, entry := range journal.Current().Journal() {
		if entry.Kind == run31.JournalAdapterAction {
			actions++
			if entry.Action != "automation.play-input-clip" || entry.ActionOutcome != run31.ActionSucceeded ||
...
func automationPlaybackSource(builtins nodes31.Builtins, slot string, ref blob.BlobRef) []byte {
	started, _ := builtins.Definition(nodes31.RunStartedNodeID)
	playback, _ := builtins.Definition(nodes31.PlayInputClipNodeID)
	return []byte(fmt.Sprintf(`{
		...
		"graphs":[{"id":"main","kind":"main","nodes":[
			...
			{"id":"playback","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},"config":{"slot":%q},
