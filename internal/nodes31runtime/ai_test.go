...
func TestExecutorRunsBoundedAIAgentToolLoopAndJournalsTerminalBudget(t *testing.T) {
	...
	source := []byte(fmt.Sprintf(`{
		...
		"revision":0,"entryGraph":"main","graphs":[{"id":"main","kind":"main","nodes":[
			{"id":"start","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":0,"y":0},"config":{},"bindings":{}},
			{"id":"agent","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},
	...
	adapters, err := nodes31runtime.Installed(builtins, dependencies)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, entry := range journal.Current().Journal() {
		if entry.Kind == run31.JournalAdapterAction && entry.NodeID == "agent" {
