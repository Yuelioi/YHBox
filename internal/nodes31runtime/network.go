...
func httpGet(builtins nodes31.Builtins) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		action := compiler.AdapterAction{EffectID: nodes31.HTTPGetEffectID, Action: "network.http-response", SummaryCode: "network.http-get", Counters: map[string]int64{}, Facts: map[string]string{}}
		defer func() {
