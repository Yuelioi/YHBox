...
func randomInteger(builtins nodes31.Builtins) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		...
		defer func() {
			runErr = errors.Join(runErr, recordAdapterOutcome(ctx, invocation, compiler.AdapterAction{
				EffectID: nodes31.RandomSampleEffectID, Action: "random.integer-sampled",
				SummaryCode: "random.integer", Counters: counters,
			}, nodes31.RandomEntropyUnavailableCode, runErr))
		}()
		minimum, err := integerInput(invocation, "minimum")
		if err != nil {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomInvalidRangeCode, err)
		}
		maximum, err := integerInput(invocation, "maximum")
		if err != nil || minimum > maximum {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomInvalidRangeCode, errors.Join(err, errors.New("minimum must not exceed maximum")))
		}
		...
		if distribution == "centered" {
			...
			for range centeredSampleCount {
				...
				if err != nil {
					return compiler.AdapterResult{}, randomFailure(nodes31.RandomEntropyUnavailableCode, err)
				}
				sum += unit
			}
			...
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomInvalidRangeCode, errors.New("unknown random distribution"))
		}
...
func randomNumber(builtins nodes31.Builtins) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		counters := map[string]int64{}
		defer func() {
			runErr = errors.Join(runErr, recordAdapterOutcome(ctx, invocation, compiler.AdapterAction{
				EffectID: nodes31.RandomSampleEffectID, Action: "random.number-sampled",
		...
		if err != nil || minimum > maximum || math.IsInf(maximum-minimum, 0) {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomInvalidRangeCode, errors.Join(err, errors.New("range must be ordered and representable")))
		}
		distribution, err := stringInput(invocation, "distribution")
		if err != nil {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomInvalidRangeCode, err)
		...
		if err != nil {
			code := nodes31.RandomEntropyUnavailableCode
			if errors.Is(err, errUnknownDistribution) {
				code = nodes31.RandomInvalidRangeCode
			}
		...
		if math.IsNaN(result) || math.IsInf(result, 0) {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomInvalidRangeCode, errors.New("sample is not a finite number"))
		}
		return sealObservedResult(builtins, invocation, result)
	}
}

func randomBoolean(builtins nodes31.Builtins) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		counters := map[string]int64{}
		defer func() {
			runErr = errors.Join(runErr, recordAdapterOutcome(ctx, invocation, compiler.AdapterAction{
				EffectID: nodes31.RandomSampleEffectID, Action: "random.boolean-sampled",
		...
		if err != nil || probability < 0 || probability > 1 {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomInvalidProbabilityCode, errors.Join(err, errors.New("probability must be between zero and one")))
		}
		unit, err := entropyUnit(invocation)
		if err != nil {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomEntropyUnavailableCode, err)
...
func randomChoice(builtins nodes31.Builtins) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		counters := map[string]int64{}
		defer func() {
			runErr = errors.Join(runErr, recordAdapterOutcome(ctx, invocation, compiler.AdapterAction{
				EffectID: nodes31.RandomSampleEffectID, Action: "random.choice-sampled",
				SummaryCode: "random.choice", Counters: counters,
			}, nodes31.RandomEntropyUnavailableCode, runErr))
		}()
		input, ok := invocation.Inputs["list"]
		if !ok {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomEmptyChoiceCode, errors.New("choice list is missing"))
		}
		var items []json.RawMessage
		if err := json.Unmarshal(input.InlineJSON(), &items); err != nil {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomEmptyChoiceCode, fmt.Errorf("decode choice list: %w", err))
		}
		if len(items) == 0 {
			return compiler.AdapterResult{}, randomFailure(nodes31.RandomEmptyChoiceCode, errors.New("choice list is empty"))
		}
...
func observeTime(builtins nodes31.Builtins) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		defer func() {
			runErr = errors.Join(runErr, recordAdapterOutcome(ctx, invocation, compiler.AdapterAction{
				EffectID: nodes31.TimeObserveEffectID, Action: "time.observed",
				SummaryCode: "time.observe", Counters: map[string]int64{"observations": 1},
			}, nodes31.TimeObservationFailedCode, runErr))
		}()
		if invocation.ObservedAt.IsZero() {
			return compiler.AdapterResult{}, &compiler.NodeFailure{Code: nodes31.TimeObservationFailedCode, Cause: errors.New("invocation observation time is missing")}
		}
		result, err := sealObservedResult(builtins, invocation, invocation.ObservedAt.UnixMilli())
		if err != nil {
			return compiler.AdapterResult{}, &compiler.NodeFailure{Code: nodes31.TimeObservationFailedCode, Cause: err}
		}
		return result, nil
	}
}
...
func sealObservedResult(builtins nodes31.Builtins, invocation compiler.Invocation, value any) (compiler.AdapterResult, error) {
...
func sealObservedRawResult(builtins nodes31.Builtins, invocation compiler.Invocation, raw json.RawMessage) (compiler.AdapterResult, error) {
