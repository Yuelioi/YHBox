...
func TestFilesystemReadJSONUsesGrantedWorkspaceProviderAndRedactedJournal(t *testing.T) {
	...
	provider, err := workspacefs.NewProvider(root, workspacefs.Limits{MaxReadBytes: nodes31.DefaultFileReadBytes})
	if err != nil {
		t.Fatal(err)
	}
	program := compilePrimitiveProgram(t, builtins, filesystemSource(t, builtins, nodes31.FileReadJSONNodeID, relative))
	...
	for _, entry := range journal.Current().Journal() {
		if entry.Kind == run31.JournalAdapterAction && entry.NodeID == "file" {
			copy := entry
			action = &copy
		}
	}
	if action == nil || action.EffectID != nodes31.FileReadEffectID || action.ActionOutcome != run31.ActionSucceeded ||
...
func TestFilesystemReadFailsClosedOnTraversalBeforeHostRead(t *testing.T) {
	...
	program := compilePrimitiveProgram(t, builtins, filesystemSource(t, builtins, nodes31.FileReadTextNodeID, "../secret.txt"))
	...
	for _, entry := range journal.Current().Journal() {
		if entry.Kind == run31.JournalAdapterAction && entry.NodeID == "file" {
			if entry.ActionOutcome != run31.ActionFailed || entry.ErrorCode != workspacefs.CodeInvalidPath {
...
func filesystemSource(t *testing.T, builtins nodes31.Builtins, nodeID, path string) []byte {
	t.Helper()
	...
	if nodeID == nodes31.FileReadTextNodeID {
		config = `{"encoding":"utf-8","maxBytes":1048576}`
	} else if nodeID == nodes31.FileStatNodeID {
		config = `{}`
	}
	return []byte(fmt.Sprintf(`{
		...
		"revision":0,"entryGraph":"main","graphs":[{"id":"main","kind":"main","nodes":[
			...
			{"id":"file","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},
