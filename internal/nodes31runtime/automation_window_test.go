...
func TestActivateWindowUsesDedicatedInstalledAuthorityAndJournal(t *testing.T) {
	...
	for _, entry := range journal.Current().Journal() {
		if entry.Kind == run31.JournalAdapterAction {
			actions++
			if entry.Action != "automation.activate-window" || entry.Summary.Code != "automation.activate-window" || entry.ActionOutcome != run31.ActionSucceeded {
...
func TestActivateWindowFailureIsNotReportedAsCompleted(t *testing.T) {
	...
	for _, entry := range journal.Current().Journal() {
		if entry.Kind == run31.JournalAdapterAction && entry.ActionOutcome == run31.ActionSucceeded {
			t.Fatalf("failed activation recorded success: %#v", entry)
		}
	}
}

func executeWindowActivation(t *testing.T, provider *automationWindowProvider) (*run31.JournalWriter, error) {
	t.Helper()
	...
	capabilityDefinition, ok := builtins.Catalog.LookupCapability(nodes31.AutomationWindowCapabilityID)
	if !ok {
...
func automationWindowSource(t *testing.T, builtins nodes31.Builtins, slot string) []byte {
	t.Helper()
	started, _ := builtins.Definition(nodes31.RunStartedNodeID)
	activate, _ := builtins.Definition(nodes31.ActivateWindowNodeID)
	return []byte(fmt.Sprintf(`{
		...
		"graphs":[{"id":"main","kind":"main","nodes":[
			...
			{"id":"activate","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},"config":{"slot":%q},"bindings":{}}
