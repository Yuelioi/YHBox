...
func writeLog(emitter LogEmitter) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		action := compiler.AdapterAction{
			EffectID: nodes31.LogWriteEffectID, Action: "observability.log-written", SummaryCode: "observability.log",
			Counters: map[string]int64{}, Facts: map[string]string{},
		}
		defer func() {
			runErr = errors.Join(runErr, recordAdapterOutcome(ctx, invocation, action, nodes31.LogWriteFailed, runErr))
		}()
		envelope, ok := invocation.Inputs["message"]
		if !ok || len(envelope.InlineJSON()) == 0 {
			return compiler.AdapterResult{}, logFailure(nodes31.LogContractError, errors.New("log message is missing"))
		}
		var message string
		if err := json.Unmarshal(envelope.InlineJSON(), &message); err != nil || utf8.RuneCountInString(message) > nodes31.MaxObservabilityMessageRunes {
			return compiler.AdapterResult{}, logFailure(nodes31.LogContractError, errors.New("log message is invalid"))
		}
		level := "info"
		...
		default:
			return compiler.AdapterResult{}, logFailure(nodes31.LogContractError, fmt.Errorf("log level %q is invalid", level))
		}
		digest, err := artifact.Sum("yotta/workflow-log-message/v1", []byte(message))
		if err != nil {
			return compiler.AdapterResult{}, logFailure(nodes31.LogContractError, err)
		}
		...
		if err := emitter.EmitWorkflowLog(ctx, LogEntry{
			...
			return compiler.AdapterResult{}, logFailure(nodes31.LogWriteFailed, err)
		}
...
func throwFailure() compiler.Adapter {
	return func(_ context.Context, invocation compiler.Invocation) (compiler.AdapterResult, error) {
		envelope, ok := invocation.Inputs["message"]
		if !ok || len(envelope.InlineJSON()) == 0 {
			return compiler.AdapterResult{}, &compiler.NodeFailure{Code: nodes31.ControlThrown, Cause: errors.New("workflow explicitly failed")}
		}
		var message string
		...
		return compiler.AdapterResult{}, &compiler.NodeFailure{Code: nodes31.ControlThrown, Cause: errors.New(message)}
	}
}
