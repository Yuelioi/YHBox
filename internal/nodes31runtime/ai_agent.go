...
func aiAgent(builtins nodes31.Builtins, now func() time.Time) compiler.Adapter {
	...
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		action := compiler.AdapterAction{
			EffectID: nodes31.AIAgentEffectID, Action: "ai.agent-terminal", SummaryCode: "ai.agent",
