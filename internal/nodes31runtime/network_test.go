...
func TestHTTPGetUsesInstalledOriginAndRedactsRequestJournal(t *testing.T) {
	...
	capabilityDefinition, ok := builtins.Catalog.LookupCapability(nodes31.HTTPGetCapabilityID)
	if !ok {
	...
	for _, entry := range journal.Current().Journal() {
		if entry.Kind != run31.JournalAdapterAction || entry.NodeID != "fetch" {
			continue
		}
		if entry.ActionOutcome != run31.ActionSucceeded || entry.Summary.Counters["status_code"] != 200 || entry.Summary.Facts["path_digest"] == "" {
...
func httpSource(t *testing.T, builtins nodes31.Builtins) []byte {
	t.Helper()
	started, _ := builtins.Definition(nodes31.RunStartedNodeID)
	fetch, _ := builtins.Definition(nodes31.HTTPGetNodeID)
	return []byte(fmt.Sprintf(`{
		...
		"graphs":[{"id":"main","kind":"main","nodes":[
			...
			{"id":"fetch","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},"config":{"slot":"http-test"},
