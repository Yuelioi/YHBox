...
func TestAutomationInputUsesInstalledTargetAndRedactsJournal(t *testing.T) {
	...
	capabilityDefinition, ok := builtins.Catalog.LookupCapability(nodes31.AutomationInputCapabilityID)
	if !ok {
...
func automationInputSource(t *testing.T, builtins nodes31.Builtins, slot string) []byte {
	t.Helper()
	started, _ := builtins.Definition(nodes31.RunStartedNodeID)
	typeText, _ := builtins.Definition(nodes31.TypeTextNodeID)
	return []byte(fmt.Sprintf(`{
		...
		"graphs":[{"id":"main","kind":"main","nodes":[
			...
			{"id":"type","nodeRef":{"nodeTypeId":%q,"semanticDigest":%q},"position":{"x":1,"y":0},"config":{"slot":%q},
