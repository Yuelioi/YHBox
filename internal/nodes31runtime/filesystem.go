...
func fileRead(builtins nodes31.Builtins, structured bool) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		action := compiler.AdapterAction{
			EffectID: nodes31.FileReadEffectID, Action: "filesystem.file-read", SummaryCode: "filesystem.read",
...
func fileStat(builtins nodes31.Builtins) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		action := compiler.AdapterAction{
			EffectID: nodes31.FileStatEffectID, Action: "filesystem.file-statted", SummaryCode: "filesystem.stat",
			Counters: map[string]int64{}, Facts: map[string]string{},
		}
		defer func() {
			runErr = errors.Join(runErr, recordAdapterOutcome(ctx, invocation, action, workspacefs.CodeReadFailed, runErr))
...
func fileReadBudget(config map[string]any) (int, error) {
	...
	if !exists {
		return nodes31.DefaultFileReadBytes, nil
	}
	maximum, err := configInt64(value)
	if err != nil || maximum < 1 || maximum > nodes31.DefaultFileReadBytes {
...
func sealFileOutputs(builtins nodes31.Builtins, invocation compiler.Invocation, raw map[string]json.RawMessage) (map[string]datatype.ValueEnvelope, error) {
