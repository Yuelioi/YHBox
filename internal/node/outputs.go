...
//
// Dynamic output nodes have their complete exit set bound outside the static
// Outputs list, so the builder cannot validate membership from Spec alone.
func validateExitName(spec *Spec, name string) {
	if HasDynamicPortRole(spec, DynamicPortOutput) {
		return
	}
