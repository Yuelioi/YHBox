...
func scriptExecute(builtins nodes.Builtins, runtime ScriptExecutor) nodeadapter.Adapter {
	return func(ctx context.Context, invocation nodeadapter.Invocation) (_ nodeadapter.AdapterResult, runErr error) {
		...
		if !ok || source == "" || len(source) > scriptengine.MaxSourceBytes {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeSourceInvalid, errors.New("script source is invalid"))
		}
		...
		if err != nil || timeout < scriptengine.MinTimeoutMillis || timeout > scriptengine.MaxTimeoutMillis {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeContractViolation, errors.New("script timeout is invalid"))
		}
		input, ok := invocation.Inputs["input"]
		if !ok || len(input.InlineJSON()) == 0 {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeContractViolation, errors.New("script input is missing"))
		}
		if invocation.InvocationID == "" || invocation.Attempt < 1 || invocation.ObservedAt.IsZero() || invocation.ReadEntropy == nil {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeRunnerCrashed, errors.New("script invocation facts are incomplete"))
		}
		digest, err := artifact.Sum("yotta/script-source/v1", []byte(source))
		if err != nil {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeSourceInvalid, err)
		}
		...
		if err := invocation.ReadEntropy(seed); err != nil {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeRunnerCrashed, fmt.Errorf("read script seed: %w", err))
		}
		...
		if err != nil {
			...
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeRunnerCrashed, err)
		}
		if err := response.Validate(); err != nil || response.AttemptID != request.AttemptID {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeRunnerProtocolViolation, errors.Join(err, errors.New("script response identity is invalid")))
		}
		if response.Outcome == scriptengine.OutcomeFailed {
			return nodeadapter.AdapterResult{}, scriptFailure(response.Failure.Code, errors.New("isolated script execution failed"))
		}
		resolved, ok := invocation.OutputTypes["result"]
		if !ok {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeContractViolation, errors.New("script result type is unresolved"))
		}
		envelope, err := datatype.SealInlineJSON(builtins.Catalog, resolved, response.Output)
		if err != nil {
			return nodeadapter.AdapterResult{}, scriptFailure(scriptengine.CodeContractViolation, fmt.Errorf("seal script result: %w", err))
		}
