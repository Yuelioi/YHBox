...
func automationTemplate(builtins nodes.Builtins, nodeTypeID string) compiler.Adapter {
	return func(ctx context.Context, invocation compiler.Invocation) (_ compiler.AdapterResult, runErr error) {
		...
		if err != nil || threshold < 0 || threshold > 1 {
			return compiler.AdapterResult{}, templateFailure(nodes.VisionMatchFailedCode, errors.Join(err, errors.New("threshold must be between 0 and 1")))
		}
		region, err := visionRegionInput(invocation)
		if err != nil {
			return compiler.AdapterResult{}, templateFailure(nodes.VisionRegionInvalidCode, err)
		}
		timeout, poll, settle, err := templateDurations(invocation, nodeTypeID)
		if err != nil {
			return compiler.AdapterResult{}, templateFailure(installed.CodeInvalidRequest, err)
		}
		templateRef, err := visionBlobInput(invocation, "template")
		if err != nil {
			return compiler.AdapterResult{}, templateFailure(nodes.VisionTemplateInvalidCode, err)
		}
		...
		if err != nil {
			return compiler.AdapterResult{}, templateNodeFailure(err)
		}
		...
		case nodes.ClickTemplateNodeID:
			...
			if settle > 0 {
				...
				if relocateErr != nil {
					return compiler.AdapterResult{}, templateNodeFailure(relocateErr)
				}
				counters["captures"]++
...
func templateDurations(invocation compiler.Invocation, nodeTypeID string) (time.Duration, time.Duration, time.Duration, error) {
...
func waitForTemplateState(ctx context.Context, invocation compiler.Invocation, templateBytes []byte, region visionRegion, threshold float64, timeout, poll time.Duration, wantPresent bool) (visionMatchResult, int, error) {
...
func captureAndMatch(ctx context.Context, invocation compiler.Invocation, templateBytes []byte, region visionRegion, threshold float64) (visionMatchResult, int64, error) {
...
func captureTemplateFrame(ctx context.Context, invocation compiler.Invocation) (_ []byte, runErr error) {
...
func clickTemplateMatch(ctx context.Context, invocation compiler.Invocation, match visionMatchResult) (runErr error) {
...
func templateResult(builtins nodes.Builtins, invocation compiler.Invocation, match visionMatchResult, output string) (compiler.AdapterResult, error) {
	...
	if err != nil {
		return compiler.AdapterResult{}, templateFailure(installed.CodeContractViolation, err)
	}
...
func templateFailure(code string, cause error) error {
	return &compiler.NodeFailure{Code: code, Output: "failed", Cause: fmt.Errorf("template automation: %w", cause)}
}
