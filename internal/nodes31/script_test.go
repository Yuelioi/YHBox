...
func TestScriptExecuteContractHasExplicitSignalsAndGeneratedCodeEditor(t *testing.T) {
	...
	if len(contract.Execution.Effects) != 1 || contract.Execution.Effects[0] != nodes31.ScriptExecuteEffectID ||
	...
	projected, ok := projection.Node(nodes31.ScriptExecuteNodeID)
