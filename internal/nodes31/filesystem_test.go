...
func TestFilesystemNodesUseExactWorkspaceCapabilityAndExplicitSignals(t *testing.T) {
	...
	capabilityDefinition, ok := builtins.Catalog.LookupCapability(nodes31.FilesystemReadCapabilityID)
	if !ok {
	...
	for _, nodeID := range []string{nodes31.FileReadTextNodeID, nodes31.FileReadJSONNodeID, nodes31.FileStatNodeID} {
	...
	if builtins.FileMetadataType.TypeRef().TypeID != nodes31.FileMetadataTypeID {
