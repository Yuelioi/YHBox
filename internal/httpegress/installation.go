...
const workflowConsentDomain = "yotta/http-egress-workflow-consent/v1"
...
func Install(drafts []InstallationDraft) (Installations, error) {
	...
	for _, draft := range ordered {
		...
		if draft.Consent != "" && draft.Consent != expected {
			return Installations{}, fmt.Errorf("HTTP installation slot %q has stale workflow consent", draft.Slot)
		}
...
func WorkflowConsentDigest(slot string, profile Profile) (artifact.Digest, error) {
	if !slotPattern.MatchString(slot) || !profile.Valid() {
		return "", errors.New("HTTP workflow consent identity is invalid")
	}
	raw, err := artifact.Marshal(map[string]any{"slot": slot, "profileDigest": profile.Digest(), "providerAbi": ProviderABI, "operations": []string{OperationGet}})
	if err != nil {
		return "", err
	}
