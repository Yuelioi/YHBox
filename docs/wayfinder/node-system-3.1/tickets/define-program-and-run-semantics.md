...
`internal/runid`、`internal/capability.RunGrant`、`internal/run.Record/Store/Owner/GrantAuthorizer` 已实现上述 identity、状态、CAS、重启中断、durable value 与 resource lifecycle 内核。尚未完成的唯一运行链工作是 NodeAttempt/AdapterAction journal、Target Planner/Policy admission、全频道 Program lowering、production interpreter composition 与旧 ContainerRunner 删除；这些缺口不得由 dual-write、legacy read 或 fallback runtime 填补。
