...
Capability Definition、Requirement、sealed attributed Plan 与 Run Grant 已进入 `internal/capability`；Node Contract 使用 exact requirements，Catalog 绑定 definitions，Compiler 把 plan artifact 写入 Program identity。Source 的自由 `requestedCapabilities` 与 preview string-grant 参数已删除。`internal/admission` 已实现 content-addressed Host Profile、跨 requirement 的 target-slot candidate intersection、显式 target/credential selection、platform/provider/capability/ABI/artifact planning、统一 Policy decision 与 durable QUEUED creation。RunRecord 内嵌 non-secret Grant artifact；Run Owner 复验实际安装 provider lock，`internal/run.GrantAuthorizer` 在每次 Broker open/borrow/call 核对完整 attribution，并注入 canonical scope 与 credential metadata。conversion tracer 已删除手工 binding/Grant 路径。AI credential 已由 `internal/securestore` 的 OS adapter 持有，前端只接收 masked availability；production composition 安装 exact provider/target/credential binding，不存在字符串 allowlist、ambient service 或内存 Grant fallback。
