...
|---|---|---|
| MCP server | 未授权输入/窗口操作 | 只监听 loopback；危险执行需要 live `Armed`；`run_node` 串行、busy gate、HWND 重校验 |
...
Yotta 和 goja Script 都不是权限隔离边界。能编辑并运行 graph/script 的作者可以组合已绑定节点执行文件、网络和进程操作；能修改本地二进制、数据目录或以同一 OS 用户执行代码的攻击者也已在信任边界内。MCP armed 是防误操作闸门，不是针对恶意本地管理员的认证系统。
