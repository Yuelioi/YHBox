...
| Script/Process/Wasm guest | 任意代码、宿主逃逸、资源滥用 | Windows 通过独立 worker/runner、AppContainer + LPAC + Job、bounded protocol、内存/时间预算和 owner 终止；Wasm runner 不启用 WASI 或 ambient import。非 Windows 当前不降级到普通子进程，而是报告 isolation unavailable |
...
| Network/Application Target | 私网访问、任意进程启动或终止 | 只执行用户配置的 origin/path/argv 和 timeout/size；配置本身就是授权，Run 快照后直接调用，不伪装成 capability consent 或 identity pinning |
...
Configured Network、Application 和 Automation Target 使用当前进程权限直接执行，没有逐节点 consent/
grant/TTL；用户配置本身就是授权。不得以“加固”为由把通用扫描或重复授权重新插回每个 Node 的热路径。
Script/Process/Wasm 隔离用于缩小 guest authority 和故障域，不对恶意本地管理员提供安全边界。
