...
| 溜鱼                   | 5s           | 5~33s | 普通鱼 5s / 紫鱼 8s / 金鱼 13s / 牛逼鱼 30s（0.1% 概率, 其实是bug） |
| 结算                   | 2s           | 1~7s  | 金鱼有过场动画 2-5s                                                 |
...
- 正常路径: cast 1s + wait 5s + hook 1s + fight 6s + settle 2s ≈ **15s**
...
模板元数据在 [tools/fish/configs/zh/templates.toml](../../tools/fish/configs/zh/templates.toml)（embed 进 exe），文件名只是引用 key：
...
**关键时序差异**：键盘 hold 是 down→up 间隔（150ms 让 InputComponent 采到），鼠标 hold 是 up 之后的等待（DOWN/UP 必须紧贴才被 Slate 识别为 click）。详见 [architecture.md §3.3-3.4](architecture.md)。
...
## 9. 模板系统（tools/fish/configs/zh/templates.toml）

模板元数据集中在 [tools/fish/configs/zh/templates.toml](../../tools/fish/configs/zh/templates.toml) 里，
TOML grouped 格式，PNG 文件路径相对 templates/ 子目录。

**templates.toml 一项的结构**：

```toml
[[fish.hook_text]]                  # [[<工具>.<slot>]]，slot 自动推断
file = "hook_text_720.png"        # 路径相对 templates/ 子目录
...
1. 截图游戏全屏 → 量出 bbox → 抠图保存为 `tools/<tool>/configs/zh/templates/<slot>_<H>.png`
2. 在 `tools/fish/configs/zh/templates.toml` 加 `[[<tool>.<slot>]]` 块
...
| F 按了没反应                | 后台输入失败                           | 见[architecture.md §3](architecture.md) PostMessage 踩坑           |
...
单流程的离线测试现在走 [pkg/capture Mock backend](development.md#调参)（v2.4 删了 `-flow` CLI dry-run）。
