...
- legacy Container graph/package 不属于 3.1 可读格式，也没有迁移承诺；3.1 编辑器切换完成后整体删除。
...
- `8cdff7e2` 起生产 composition 不再装配 legacy `ContainerRunner`、旧 debug manager 或旧 MCP HTTP server。现有 legacy Container Run/Stop/Debug RPC 不再是可执行入口；GUI、CLI、MCP、debug、schedule 与 hotkey 必须逐项切到同一个 Application/Program Snapshot 命令面。
